...
const MobileData = ({ Bundles, imageRef, network }) => {
  //States values
  ...
  return (
    <main className="relative h-full">
      ...
      <div className="w-[90%] mx-auto">
        ...
        <img
          src={imageRef}
          className="w-2/5 md:w-1/5 mx-auto mt-3 mb-1"
          alt="Main Image"
        />
        ...
        <div className="w-full md:w-4/5 mx-auto">
          ...
          <div className="h-[35vh] md:h-[40vh] overflow-y-scroll overflow-x-hidden grid grid-cols-2 md:grid-cols-4 xl:grid-cols-6 gap-6 px-4 py-5 border-[0.18rem] border-t-0 border-slate-100 rounded-b-lg select-none">
...
export async function getServerSideProps({ query: { bill_id } }) {
  ...
  const BundlesArray = result.filter(
    (item) => item.biller_name.startsWith(bill_id) && item.country == "NG"
  );
